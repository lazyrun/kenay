#ifndef PERS_REPORTER
#define PERS_REPORTER
   ...
   */
   class reporter : public tut::callback
   {
      ...
      public:
         ...
         reporter() : os(std::cout), errlog(std::cout)
         {
            init();
         }

         reporter(std::ostream& out) : os(std::cout), errlog(out)
         {
            init();
         }
         ...
         void run_completed(std::ostream& os)
         {
            os << std::endl;
         ...
         void run_completed()
         {
            if(!multi_passed)
            {
               run_completed(os);
               if((&os != &errlog) &&
                  ((terminations_count > 0) || (exceptions_count > 0) ||
                   (failures_count     > 0) || (warnings_count   > 0)))
               {
                  run_completed(errlog);
               }
            }
         }
